# 手写决策树  也写了预测  没有画图,只做了全部分类之后才停止,ID3本身算法就没有剪枝操作，现在修改已经完成
"""
ID3缺点：
        1.没有剪枝策略，容易过拟合
...
def cal_info_gain(x,y):
    ...
    for i in range(x.shape[1]):
        ...
        info_gain = init_entropy - condition_entropy
        if info_gain > max_value: # 如果大于当前最大的信息熵，则更新max_value和max_feature
            max_value = info_gain
            max_feature = condition_x.name
    return max_feature
